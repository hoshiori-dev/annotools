...
compatibility: Scripts need Python 3.12+ with the standard library; parquet export needs pyarrow.
---
...
4. **Give the execution agent three tools**: `record_annotation`, `update_annotation`,
   ...
   [references/tool-contract.md](references/tool-contract.md) when implementing them.
5. **Export**: `python scripts/export.py --db … --format jsonl --out output/` writes one line per item
   with its final annotations ([scripts/export.py](scripts/export.py)); `csv`, `parquet`, and
   `webdataset` (tar of `<key>.<ext>` + `<key>.json`) follow the same selection.
...
segment `{"start": s, "end": s, "label": "..."}`. Views `final_annotations` and `items_pending`
serve the common queries.
