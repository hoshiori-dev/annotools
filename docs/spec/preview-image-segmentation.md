...
present), and in legend mode `legend: [{"id", "name", "color"}]`.

## Behavior

1. Load the mask; it must decode to mode `L`, `I;16`, `I`, or `P` with one channel — anything else
   raises `ValueError("mask_source: …")`.
2. Resize the mask to the source size (nearest neighbour) if it differs, apply the same crop as the
   image, and resize to the output size (nearest).
...
5. `label`: draw the ID (or its `id_names` entry) as a tag at the region centroid, clamped inside the
   ...
   columns), then re-fit the combined image inside `max_width` × `max_height` (nearest for the mask
   already blended — plain resize of the composite).
...
6. `test_ac6_tool`: via `Client(mcp)` with grid → metadata has `grid` and `ids == 2`.
