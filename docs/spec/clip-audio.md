...
1. Open the source with PyAV (`annotools[media]`; missing → `ImportError` naming the extra); a source
   without an audio stream raises `ValueError` naming it. Remote sources are read into memory first.
...
- Error: `start < 0`, `end ≤ start`, `sample_rate < 1` → `ValueError`; `start` beyond the source
  duration → `ValueError`.
...
4. `test_ac4_save_to`: `save_to` writes bytes identical to the returned audio.
5. `test_ac5_tool`: `Client(mcp)` returns an `AudioContent` (`audio/wav`) then a JSON text block with
   `duration ≈ 3`.
