...
wheel to TestPyPI, which is a rehearsal channel rather than an install path. A full release then
continues to PyPI with that same wheel. A `nightly` image is built from main daily.
