...
the Codex SDK (`openai-codex`), using annotools previews for cheap image input and SQLite as the store. This
...
- `src/captioning/` — pipeline (`pipeline.py`), the tools served to the agent as a FastMCP stdio server (`tools.py`), store access (`store.py`)
