"""SQLite access for the detection pipeline (schema from the sqlite-annotation-store skill)."""
...
def record(
    ...
    cur = conn.execute(
        "INSERT INTO annotations(item_id, run_id, kind, key, label, payload_json, confidence, rounds, status) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "
        "ON CONFLICT(item_id, run_id, kind, key) DO UPDATE SET label = excluded.label, "
        "payload_json = excluded.payload_json, confidence = excluded.confidence, rounds = excluded.rounds, "
        "status = excluded.status "
        "RETURNING id",
        (item_id, run_id, kind, key, label, json.dumps(payload), confidence, rounds, status),
    )
