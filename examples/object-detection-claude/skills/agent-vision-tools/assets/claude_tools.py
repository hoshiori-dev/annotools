...
vision = VisionTools(WORKSPACE, max_width=768, max_height=768)  # Claude: 768 per the size table
...
async def look_at_item(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if grid is not None and not isinstance(grid, dict):
        return {
            "content": [{"type": "text", "text": 'grid must be an object such as {} or {"columns": 4}'}],
            "is_error": True,
        }
    try:
