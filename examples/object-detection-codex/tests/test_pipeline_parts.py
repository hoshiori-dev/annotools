...
async def test_mcp_server_contract(workspace):
    ...
    from detection.pipeline import flatten_usage, mcp_config
    from detection.tools import build_server
    ...
    async with Client(build_server(ctx)) as client:
        ...
        assert names == {"look_at_item", "propose_boxes", "commit_boxes"}
        ...
        assert json.loads(text.text)["grid"]["columns"] == 10
        boxes = [{"label": "black_cat", "box": [76.8, 51.2, 384, 256], "confidence": 0.9}]
        overlay = await client.call_tool("propose_boxes", {"uri": "data/raw/coco-cats/a.jpg", "boxes": boxes})
        assert json.loads(overlay.content[1].text)["round"] == 1
        committed = await client.call_tool(
            "commit_boxes", {"uri": "data/raw/coco-cats/a.jpg", "boxes": boxes, "done": True}
        )
